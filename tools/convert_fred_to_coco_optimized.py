"""
Script to convert FRED dataset to COCO format for MvHeatDET (Optimized version)
"""
import os
import json
import argparse
...
def convert_fred_to_coco(fred_root, output_dir, split='train'):
    ...
    Args:
        ...
        split: 'train' or 'test'
    """
    print(f"Converting FRED {split} dataset to COCO format (Optimized)...")
    ...
    # Fixed image dimensions for FRED dataset
    ...
    for seq_path in tqdm(sequences):
        ...
        for frame_idx, frame_path in enumerate(event_frames):
            ...
            # Add image info to COCO format (using fixed dimensions)
...
def main():
    parser = argparse.ArgumentParser(description="Convert FRED dataset to COCO format (Optimized)")
