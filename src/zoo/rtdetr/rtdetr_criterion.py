...
__all__ = ['SetCriterion', 'SetCriterionForVideos']
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        if len(losses) == 0:
            raise ValueError('No losses were computed. Check configured losses and weight_dict.')
...
@register
class SetCriterionForVideos(SetCriterion):
    __share__ = ['num_classes', ]
    __inject__ = ['matcher', ]
    ...
    def forward_one(self, outputs, targets):
        """ This performs the loss computation.
        Parameters:
             outputs: dict of tensors, see the output specification of the model for the format
             targets: list of dicts, such that len(targets) == batch_size.
    ...
    def forward(self, outputs, targets):
        ...
        # b=len(outputs)
        # for i in range(b):
        #     loss=self.forward_one(outputs[i], targets[i])
        #     losses.append(loss)
        losses = self.forward_one(outputs, targets)
        return losses
